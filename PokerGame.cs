...
namespace PokerGame
{
    ...
    public class Player
    {
        ...
        // Function to initialize the revolver
        public void InitializeRevolver()
        {
            revolver.Clear(); // Clear the revolver in case it was previously used

            // Add 6 elements, all false
            for (int i = 0; i < 6; i++)
            {
                revolver.Add(false);
            }
            ...
            // Set one random index to true
            System.Random random = new System.Random();
            int randomIndex = random.Next(0, revolver.Count - 1);
            revolver[randomIndex] = true;
        }


        public bool PullRevolver()
        {
            if (revolver.Count == 0)
            {
                throw new InvalidOperationException("Revolver is empty!");
            }

            // Get the first value
            bool firstValue = revolver[0];
        ...
        public static async Task DelayedFunctionCall()
        {
            await Task.Delay(2000); // Delay for 2000 milliseconds (2 seconds)
            Console.WriteLine("Function called after delay!");
        }


        public async void TakeTurn(GameManager gameManager)
        {
            System.Random random = new System.Random();
            int randomNumber;
            await DelayedFunctionCall();

            Debug.Log(gameManager.handEvaluator.EvaluateHandToString(gameManager.listOfPlayers[1].hand));
            switch (gameManager.handEvaluator.EvaluateHandToString(gameManager.listOfPlayers[1].hand))
            {
                case string s when s.Contains("High Card"):
                    randomNumber = random.Next(1, 10);

                    if (gameManager.currentBet <= 40)
                    {
                        PerformAction("Check", gameManager);
                    }
                    else
                    {
                        if (gameManager.currentPhase == GameManager.Phase.preFlop)
                        {
                        ...
                        else
                        {
                            ...
                            {
                                PerformAction("Fold", gameManager);
                            }
                            else
                            {
                                gameManager.listOfPlayers[1].selectedRaiseValue = 40;
                                PerformAction("Raise", gameManager);
                            }
                        }
